...
class MenuScene extends Phaser.Scene {
    ...
    create() {
        ...
        const buttons = [
            { text: 'Играть', y: CONSTS.HEIGHT / 2 - 180, callback: () => this.scene.start('GameScene') },
            { text: '1v1 Онлайн', y: CONSTS.HEIGHT / 2 - 125, callback: () => this.scene.start('MatchmakingScene') },
            { text: 'Дуэли', y: CONSTS.HEIGHT / 2 - 70, callback: () => this.scene.start('DuelHistoryScene') },
            { text: 'Рейтинг', y: CONSTS.HEIGHT / 2 - 15, callback: () => this.openLeaderboard() },
            { text: '🏆 Награды', y: CONSTS.HEIGHT / 2 + 40, callback: () => this.scene.start('DailyRewardScene') },
            { text: '📊 Статистика', y: CONSTS.HEIGHT / 2 + 95, callback: () => this.scene.start('StatsScene') },
            { text: '🎁 Рефералы', y: CONSTS.HEIGHT / 2 + 150, callback: () => this.scene.start('ReferralScene') },
            { text: '🎒 Инвентарь', y: CONSTS.HEIGHT / 2 + 205, callback: () => this.scene.start('InventoryScene') },
            { text: '💎 Кошелёк', y: CONSTS.HEIGHT / 2 + 260, callback: () => this.scene.start('WalletScene') },
            { text: '⭐ Магазин', y: CONSTS.HEIGHT / 2 + 315, callback: () => this.openWebShop() },
        ];
...
const config = {
    ...
    scene: [MenuScene, LeaderboardScene, InventoryScene, StatsScene, WalletScene, DailyRewardScene, ReferralScene, MatchmakingScene, DuelHistoryScene, GameScene]
};
